	...
	if [ -z "$log_to_syslog" ] || { [ -n "$log_to_syslog" ] && [ "$log_to_syslog" = true ]; }; then # 1/
		if command -v system_logger &>/dev/null; then # 2/
			...
			if command -v logger &>/dev/null; then # 3/
				logger -t "$program_name" "Builder: $1"
				return 0
			fi # 3/
		fi # 2/
	fi # 1/
...
case "$fan_control_method" in # 1/
	direct|universal|zone)
	;;

	group)
		debug_print 4 "IPMI write fan control method is \"$fan_control_method\""
		if [ "${reserved_fan_name[@]}" -eq 0 ]; then # 1/
			reserved_fan_name[dummy]=0
			reserved_fan_name[override]=0
			debug_print 1 warn "Auto-generated special fan placeholder array (\$reserved_fan_name) because it was not declared in config as expected"
		fi # 1/
	;;
